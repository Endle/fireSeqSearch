fireSeqSearch (pre-LLM): Append Logseq/Obsidian notes while Googling
====================================================================

> **This is the pre-LLM version of fireSeqSearch.** It uses tantivy/BM25 lexical
> search and has no embedding model or RAG dependencies — a single binary, no
> GPU, no llama-server.
>
> The current `master` branch is LLM-first (semantic retrieval over `bge-m3` +
> per-page LLM summaries + a `/ask` RAG endpoint). If you want this lighter
> version, use release `0.9` or build from source at that tag. New features
> land on `master`; this doc is
> preserved for reference and as install instructions for `0.9`.
